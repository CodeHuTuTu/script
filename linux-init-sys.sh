...
#---------------------------------------------------------------------------
# 自动刷新环境变量（无需手动 source）
#---------------------------------------------------------------------------
log_info "正在自动刷新环境变量..."

# 在当前 shell 中 source 所有修改过的 bashrc
for bashrc in "${bashrc_files[@]}"; do
    if [[ -f "$bashrc" ]]; then
        set +e
...
# 立即导出 vim 相关编辑器环境变量，确保当前 shell 会话立即生效
export EDITOR=vim
export VISUAL=vim
log_info "✓ 编辑器环境变量已生效 → EDITOR=vim  VISUAL=vim"

# 对指定用户：通过 runuser 在其登录 shell 中预执行 source
# 使变量对通过本脚本派生的子进程立即可用
if [[ "$scope_choice" == "2" ]]; then
    for username in "${target_users[@]}"; do
        runuser -l "$username" -c 'source ~/.bashrc 2>/dev/null || true' 2>/dev/null || true
        log_info "✓ 已为用户 $username 预加载 ~/.bashrc"
    done
fi

log_info "✓ 环境刷新完成！当前 shell 已立即生效；其他已存在的终端会话重新登录后生效。"
}
